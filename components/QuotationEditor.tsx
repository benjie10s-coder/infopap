...
export function QuotationEditor() {
  ...
  const saveQuotation = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      let res: Response;
      if (store.currentQuotationId) {
        res = await fetch(`/api/quotations/${store.currentQuotationId}`, {
          method: "PUT",
      ...
      if (data.quotationNumber) {
        store.setField("quotationNumber", data.quotationNumber);
      }
  ...
  const handleDownload = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      a.href = url;
      a.download = `${store.quotationNumber || "quotation"}.pdf`;
      a.click();
  ...
  const handlePaymentSuccess = useCallback(() => {
    ...
    if (store.currentPublicId) {
      const a = document.createElement("a");
      a.href = `/api/documents/download-quotation/${store.currentPublicId}`;
      a.download = `${store.quotationNumber || "quotation"}.pdf`;
      a.click();
    }
  }, [store.currentPublicId, store.quotationNumber]);
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-mist safe-top">
        <div className="max-w-[1400px] mx-auto px-3 sm:px-4">
          ...
          <div className="pb-2 -mx-1">
            <DocumentTypeSwitcher variant="header" fallbackLabel="Quotation" />
          </div>
        </div>
      </header>

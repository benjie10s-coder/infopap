...
export function PurchaseOrderEditor() {
  ...
  useEffect(() => {
    ...
    if (store.currentPurchaseOrderId !== null || store.from.name.trim() !== "") return;
  ...
  }, [authLoading, profileLoading, user, preference, profile, store.currentPurchaseOrderId, store.from.name, applyPrefill]);
  ...
  const savePurchaseOrder = useCallback(async (): Promise<string | null> => {
    ...
    try {
      const payload = {
        ...
        shipToName: store.shipTo.name || undefined,
        shipToCompanyName: store.shipTo.companyName || undefined,
        shipToAddress: store.shipTo.address || undefined,
        shipToCity: store.shipTo.city || undefined,
        shipToZipCode: store.shipTo.zipCode || undefined,
        shipToPhone: store.shipTo.phone || undefined,
